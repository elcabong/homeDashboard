! function(t, e, s, n, o) {
    ...
    t.mPanel = function() {
		...
		var getDailyReset = 0;
		var weatherObj = {};
		weatherObj.lastUpdate = Date.now();
		weatherObj.current = {};
		weatherObj.daily = {};
		weatherObj.hourly = {};
		/*
		...
		*/
			...
			$.get('./data/weather.json', function(data) {
				...
				weatherObj.lastUpdate = 0;
				
			}).always(function() {

				var difDate = Date.now() - weatherObj.lastUpdate;
				
				
				
				console.log("now: " + Date.now());
				console.log("old: " + weatherObj.lastUpdate);
				console.log("diff: " + difDate);
				...
				// need to find proper time delay here for updates
				
				// might need to check when last dataUpdated was updated.. if longer than 30/60 minutes, run refresh anyway.
				

				
				if(difDate<1200000){
					// difDate closer than refresh rate, use JSON data
					console.log("data still recent, using JSON");
			...
			function updateWeatherFromObject(){
				...
				var dateVal ="/Date("+weatherObj.lastUpdate+")/";
				var date = new Date( parseFloat( dateVal.substr(6 )));
				var currentTime = 
					(date.getMonth() + 1) + "/" +
					date.getDate() + " | " +
					correctHours(date.getHours()) + ":" +
					correctMinute(date.getMinutes());				
				
				var date = new Date( current.last_ob_time + "Z" );
				var lastUpdated = 
					(date.getMonth() + 1) + "/" +
					date.getDate() + " | " +
					correctHours(date.getHours()) + ":" +
				...
				document.getElementById('weather-current').innerHTML = '<div class="bright big">'
					...
					+	'</div>'
					+   '<div><span class="weather-forecast-item-low xxsmall">Checked: '+ currentTime +'</span></div>'				
					+   '<div><span class="weather-forecast-item-low xxsmall">Updated: '+ lastUpdated +'</span></div>';
				
				
				// update daily data:
				var daily = weatherObj.daily;
				...
				for(var count = 0; count < 5; count++){
					...
					+	'<div class="bright weather-forecast-item-low xsmall">'+ daily[count].min_temp+'<sup>°</sup></div>'
					+	'</div>';
				...
				for(var count in hourly){
					...
					+	'<div class="day bright">'+formattedTime+'<span class="bright xxsmall"> '+post
				...
				e("#weather-loading").hide(), e("#weather").fadeIn(), e("#weather-container-current").fadeIn(), e(s).resize();			
			}
